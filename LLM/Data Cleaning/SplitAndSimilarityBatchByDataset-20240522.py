...
def process_file(input_file, output_dir, discarded_dir, given_dialogue, threshold=0.8, file_format='json', batch_size=64):
    ...
    for batch_index, batch_texts in enumerate(tqdm(dataloader, desc=f"Processing {os.path.basename(input_file)}")):
        ...
        for i, is_similar in enumerate(similarities):
            ...
            if is_similar:
                discarded_file_path = os.path.join(discarded_dir, f"{base_name}.{file_format}")
                with open(discarded_file_path, 'a', encoding='utf-8') as f:
                    if file_format == 'json':
                        json.dump(batch_items[i], f, ensure_ascii=False)
                    elif file_format == 'jsonl':
                        f.write(json.dumps(batch_items[i], ensure_ascii=False) + '\n')
            else:
                output_lang_dir = os.path.join(output_dir, f"{base_name}_{lang}")
                os.makedirs(output_lang_dir, exist_ok=True)
                output_file_path = os.path.join(output_lang_dir, f"{base_name}_{lang}.{file_format}")
